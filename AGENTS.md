...
- Set and record random seeds. (Note: sub-seeds from the run seed through a stable digest, not through Python's per-process-salted `hash()`.) Prove reproducibility: after any change to the numerical path, run the smallest real-model config twice and diff.
...
GPU runs go through the `run-gpu` workflow (see `README.md`): request one by editing `.github/gpu-run.yaml`, committing and pushing (the workflow triggers on pushes that change that file; a manual `workflow_dispatch` is the fallback and needs Actions write permission, which agent sessions do not have). Follow the run with `gh run watch` and fetch the artifact with `gh run download`. Be sure to think through the GPU tier yourself (the following heuristics are standard):
