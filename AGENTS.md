...
- Check the credit before a long run: `uv run python scripts/runpod_balance.py --hours <h> --rate <usd/h>` must report `ok` (estimate plus a 50 % margin); do not request the run otherwise. A job submitted without credit is refused (HTTP 402) and one that exhausts it mid-run is lost.
