...
With the vector injected at \(l^*\), at every later read point \(m\) the perturbation \(\delta_m\) at the query token is edited on top of the injection and the held-out effect that survives is measured on the first `commitment.n_prompts` evaluation prompts: **remove** takes out the component along a direction \(u\) (\(\delta_m \leftarrow \delta_m - (\delta_m \cdot u)\,u\)); **keep** leaves only that component. \(u\) is the injected direction and, separately, the task's PC1 at \(m\). Each edit is matched against the same edit along `commitment.n_controls` random unit directions on the same steered run (paired excess test). The summary per direction: the last read point at which removing it costs effect beyond random removal, the first read point after it (the commitment layer, also as a fraction of the downstream depth), the effect retained after removal at the end, and the share the direction alone carries at the end (`commitment.json`, `qualification.json/commitment`).
