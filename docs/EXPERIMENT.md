...
Every steered pass also records a damage measure (D24): the KL divergence from the unsteered next-token distribution at the query token and whether the most likely token changed, for every grid point (with its random screen), for the selected condition and every control kind on the held-out pool (paired excess tests against the controls at the same norm), and for the strength-robustness alternatives. It is reported, not a gate.
