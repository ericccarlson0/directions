...
### Depth of commitment (docs/DECISIONS.md D29)

With the vector injected at \(l^*\), at every later read point \(m\) the perturbation \(\delta_m\) at the query token is edited on top of the injection and the held-out effect that survives is measured on the first `commitment.n_prompts` evaluation prompts: **remove** takes out the component along a direction \(u\) (\(\delta_m \leftarrow \delta_m - (\delta_m \cdot u)\,u\)); **keep** leaves only that component. \(u\) is the injected direction and, separately, the task's PC1 at \(m\). Each edit is matched against the same edit along `commitment.n_controls` random unit directions on the same steered run (paired excess test). The summary per direction (`commitment.json`, `qualification.json/commitment`): the hand-over depths as effect sizes (core; `commitment.handover_shares`, default 0.5 and 0.9): the first read point from which removing the direction leaves at least that share of the effect at every later read point (`handed_over_50`, `handed_over_90`) and the last read point up to which the direction alone carries at least that share (`carried_alone_until_50`, `_90`), each also as a fraction of the downstream depth; and, secondary, the last read point at which removing it costs effect beyond random removal, the first read point after it (the commitment layer), the effect retained after removal at the end, and the share the direction alone carries at the end.

Amended (D35): beside the shape-matched random edits, each real edit is repeated with its own per-prompt magnitude \((\delta_m \cdot u)\) along the random directions (`random_matched_*`; `commitment.matched_controls`). A read point is *readable* when both random nulls keep their premise (random removal retains the effect within `commitment.readability_tolerance` of 1, random keeping alone within it of 0); the hand-over depths are read over the readable read points only and the unreadable ones are listed (`unreadable_read_points`). Gemma 4's deep half is unreadable by any residual edit of the sizes these tests use (`scripts/probe_edit_sensitivity.py` maps the sensitivity); Qwen3 and OLMo 3 are readable throughout.

### Trajectories compared all-to-all (docs/DECISIONS.md D32)

A separate command, `directions trajectories`, reads two finished runs of one model (the head-mean run and the learned-vector run: same seed, splits and held-out prompts) and compares the *downstream trajectories* of the three constructions of a task direction with one another and with the natural one. For each held-out zero-shot prompt \(x\) and each construction \(v\) (the pooled PC1, the head-mean function vector, the learned vector) injected at a common layer \(l\), the perturbation at every later read point is \(\delta_m(v)(x) = h_m(x; h_l + v) - h_m(x)\); the natural trajectories are \(\delta_m^{\mathrm{ICL}}(x) = h_m(x; \text{demos}) - h_m(x)\) (the pipeline's own few-shot prompts), the same with a second demonstration sample, and the label contrast \(h_m(x; \text{demos}) - h_m(x; \text{deranged demos})\) that the extraction uses. Every pair among these six is compared per example and per read point by the signed cosine (and, for the natural references, by the projection fraction \(\delta_m(v) \cdot \delta_m^{\mathrm{ICL}} / \|\delta_m^{\mathrm{ICL}}\|^2\)); the population-mean trajectories are compared too. Every cosine is also taken with the answer's unembedding direction (the final norm's scale times the first target token's unembedding row) projected out of both vectors, so that two trajectories that merely raise the same answer token do not read as one computation, and with the *generic response* projected out: the mean trajectory of the isotropic controls at the layer (per example and read point), what any perturbation of these norms does downstream, which alone aligns every late trajectory with every other (for a control's own floor, the mean of the other controls).

Iteration 9 (docs/DECISIONS.md D33) repeats the comparison at each of `strength_factors` times the canonical strength (canonical first; the others nested under `other_strengths`, with the cosine between the same construction's trajectories at the two strengths), records what the generic response is (its energy in its own and in the unsteered residual's largest coordinates, its cosine with the residual mean, its logit lens at the last read point, the coherence of the random responses, its cosine across strengths), and runs the **patch test** at the primary layer: at read points at fixed fractions of the downstream depth the steered perturbation is edited on the query token (the component along the prompt's natural difference removed, or kept alone) or the natural difference is patched into the unsteered run, each edit matched against random per-example directions, and the effect retained is reported per token and on the first target token (an edit at the last read point reaches only the query position's own prediction). All trajectory geometry runs in torch on the model's device, with the NumPy functions as the tested reference. As amended, the factors are canonical, half and quarter strength; with `neighbour_layers: 1` the nearest candidate layer below and above the primary layer is compared too, each construction at the strength its own run's calibration gives it there, and with `patch.layers: all` the patch test runs at every compared layer, so the hand-over is read per injection layer; the patch grid is eighths of the downstream depth and the head-mean vector is edited beside the learned one. The *per-block writing* of the shared component is recorded for every trajectory: the share of the natural difference \(\delta^{\mathrm{ICL}}_{m+1}\) that block \(m+1\) writes, \((\delta_{m+1} - \delta_m) \cdot \delta^{\mathrm{ICL}}_{m+1} / \|\delta^{\mathrm{ICL}}_{m+1}\|^2\), raw and with the generic response removed, summarised by the dominant block and its share, the entropy ratio of the positive shares and the rank correlation with the natural trajectory's own block profile (the causal localisation beside the geometric conversion mass). The *causal dimensionality* of the shared component (docs/DECISIONS.md D34): at the primary layer, at read points of the downstream depth and at the hand-over read point, the steered perturbation is kept only within (or stripped of) a k-dimensional subspace, and the prompt's natural difference projected on that subspace is patched alone, for k in `subspace.k_grid`; the subspace is the top-k uncentred principal components of the natural differences of the run's extraction and calibration pools (never the held-out prompts), compared with the same components of the other tasks' pools (leave-one-task-out), of the unsteered residuals (the background) and with random per-example subspaces of the same rank. The effect retained against k, its k90, the pool spectrum and the subspace overlaps are recorded. What the generic response turned out to be, and the hypotheses about it that are recorded but not pursued, are condensed in docs/GENERIC_RESPONSE.md; the generic-removed variant remains the one the results quote.

The common layers are, per task, the learned run's selected layer (primary) and the head-mean run's when it differs. Each construction enters at the strength its own run's calibration gives it at that layer (the selected point at the selected layer, else the reliable grid point nearest the reference strength, else the natural norm); PC1, which neither run calibrated, takes the grid point in units of the median residual norm with the largest mean per-token improvement on the calibration pool. `n_isotropic` matched random directions per construction and layer, at the construction's strength, give the floor of every alignment; the cosine between the two natural trajectories (different demonstration samples) gives its ceiling. Medians carry bootstrap CIs over examples. For each construction and reference the summary records the median cosine at injection, its peak and the peak's depth (fraction of the downstream depth), its final value, the floor at the same points and a paired test of the decline from the peak to the end. Three readings are distinguished: a trajectory that never clears the floor after injection (a different computation implements the task), one that rises and stays above it (downstream layers canonicalise the control signal onto the natural trajectory), and one that rises and then falls back (transient alignment: one of several routing signals). The labels attached to the curves are exploratory (the peak is chosen on the same data); the curves and the numbers are the result.
