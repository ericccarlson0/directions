...
There are two ways to start a run. The primary one needs nothing but git push
access (no Actions permission), so it works from agent sessions and scripts:
edit the request file `.github/gpu-run.yaml` (command, GPU tier, optional
pinned device, data center, Flash environment, budgets), commit, and push. The
workflow triggers on every push that modifies that file and reads it; the
request is therefore versioned with the exact code it ran. An unchanged file
does not trigger, so re-requesting an identical command needs a new `request`
label. `scripts/runpod_request.py` validates the file (`uv run pytest` checks
the committed one).

```bash
# request a run: edit the request file, commit, push (command runs at the repository root on the worker)
$EDITOR .github/gpu-run.yaml
git commit -am "request: pilot3b_qwen3_0.6b_seed1" && git push -u origin <branch>

# or dispatch manually (the form's inputs are used; the request file is ignored)
